function [eePos, eeVel, statusFlag] = fk(actuatorPos, actuatorVel)
    ...
    R = 0.364;  % sont exactement les mêmes que Rtop
    r = 0.036;  %sont exactement les mêmes que Rbot
    a = R*(sqrt(3)/2);
   % b = pi/4;
    b = pi/6;
    %h = 0.35;%%%%%%%%%%%%%%%%%%%%%%%%ùù 9sira 0.5 abrek 
    h = 0.6;
    d = r*(sqrt(3)/2);
    ...
    v = d-a;
%    disp(['Taille de actuatorPos: ', num2str(length(actuatorPos))]);
    ...
    Pn = [Pxn Pyn Pzn+bottom_plate_thick/2+eeL-based_plate_thick/2]';
         ...
         if ~isreal(Pp) || ~isreal(Pn)
                % Sécurité : Si le résultat est imaginaire, on évite le crash
                disp('Attention : Singularité détectée !');
                statusFlag = 0; % On indique que le calcul a échoué
                eePos = [0; 0; -0.3]; % On donne une valeur par défaut "saine"
                eeVel = [0; 0; 0];
                return; % On sort de la fonction proprement
       %    elseif real(Pp(3)) >= 0
            elseif real(Pp(3)) <= 0
                statusFlag = 1;
                eePos = real(Pp);
    ...
    % Force actuatorVel à être un vecteur colonne (3x1)
